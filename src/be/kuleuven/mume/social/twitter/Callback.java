...
public class Callback extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			RequestToken requestToken = (RequestToken) convertToObject((Blob) temp
					.getProperty("rtoken"));
			Twitter twitter = (Twitter) convertToObject((Blob) temp
					.getProperty("twitter"));
			String googleId = (String) temp.getProperty("UserId");

			if (requestToken == null) {
				log.log(Level.SEVERE,
						"Callback: No requestToken attribute not found!;");
				return;
			}
			...
			Key key = KeyFactory.createKey(Persoon.class.getSimpleName(),
					googleId);
	...
	public static void test(Persoon persoon, PersistenceManager pm,
		...
		try {
			...
			log.log(Level.INFO,
					"Searches Remaining: " + status.getRemainingHits());
		} catch (TwitterException e) {
	...
	private Object convertToObject(Blob b) throws IOException,
